public class ChessBoard {
    ...
    public boolean moveToPosition(int startLine, int startColumn, int endLine, int endColumn) {
        if (checkPos(startLine) && checkPos(startColumn)) {
            if (board[startLine][startColumn] == null) return false; //проверяем, чтобы в начальной позиции что-то было
...
/*
        ...
        if (nowPlayer.equals("White")) {
            ...
            if (board[7][7].getSymbol().equals("R") && board[7][4].getSymbol().equals("K") && // check that King and Rook
                ...
                if (board[7][7].getColor().equals("Black") && board[7][4].getColor().equals("Black") &&
                    ...
                    //board[7][4] = null;
                    board[7][6] = board[7][4];//new King("Black");   // move King
                    board[7][4] = null;
    ...
    public boolean castling0() {
        return canCastling(true);
    }

    public boolean castling7() {
        return canCastling(false);
    }
    private boolean canCastling(boolean left){
        //String color = nowPlayer;
        int column = left ? 0 : 7;
        int newColumKing = left ? 2 : 6;
        int newColumRook = left ? 3 : 5;
        if (!nowPlayer.equals("White") && !nowPlayer.equals("Black")) return false;
        int line = nowPlayer.equals("White") ? 0 : 7;
        if (board[line][column] == null || board[line][4] == null) return false;
        if (board[line][column].getSymbol().equals("R") && board[line][4].getSymbol().equals("K") && // check that King and Rook
                checkEmptyLine(line,column,4)) {              // never moved
            if (board[line][column].getColor().equals(nowPlayer) && board[line][4].getColor().equals(nowPlayer) &&
                    board[line][column].check && board[line][4].check &&
                    !new King(nowPlayer).isUnderAttack(this, line, newColumKing)) { // check that position not in under attack
                board[line][newColumKing] = board[line][4];   // move King
                board[line][4] = null;
                board[line][newColumKing].check = false;
                board[line][newColumRook] = board[line][column];   // move Rook
                board[line][column] = null;
                board[line][newColumRook].check = false;
    ...
    private boolean checkEmptyLine(int line, int column1, int column2){
